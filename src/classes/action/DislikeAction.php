    ...
    public function execute(): string
    {
        if ($_SERVER['REQUEST_METHOD'] == 'GET'){
            if (isset($_SESSION['user'])) {
                ...
                // On vérifie si l'utilisateur a déjà mis un like ou un dislike
                ...
                // On modifie la note du touite en fonction de si l'utilisateur a déjà mis un like ou un dislike
                if ($verifieLike == 0 && $verifieDislike == 0) {
                    ...
                    $addUser = $bdd->prepare("INSERT INTO ADISLIKE VALUES (:email, :id)");
                    $addUser->bindValue(":email", $email);
                    $addUser->bindValue(":id", $_GET['id']);
                    $result = $addUser->execute();

                    //s'il a deja mis un like on enleve le like et on met un dislike
                } elseif ($verifieLike == 1 && $verifieDislike == 0) {
                    $req = $bdd->prepare("SELECT note FROM TOUITE WHERE id_touite = :id");
                    $req->bindValue(":id", $_GET['id']);
                    $result = $req->execute();
                    $ancienneVal = $req->fetchColumn();
                    ...
                    //s'il a deja mis un dislike on enleve le dislike
                ...
                // On redirige l'utilisateur vers la page d'où il vient
